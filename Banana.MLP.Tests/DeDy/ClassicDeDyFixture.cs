...
using Banana.MLP.DeDyAggregator;
...
namespace Banana.MLP.Tests.DeDy
{
    [TestClass]
    public class ClassicDeDyFixture
    {
        ...
        public void TestCPU()
        {
            ...
            for (var seed = 0; seed < 1000; seed += rnd.Next(25))
            {
                ...
                int maxDiffIndex;
                float maxDiff;
                if (!ArrayHelper.ValuesAreEqual(csharp, cl, 1e-9f, out maxDiff, out maxDiffIndex))
                {
                    Console.WriteLine(
                        "Diff {0} at index {1}",
                        maxDiffIndex,
        ...
        {
            var randomizer = new ArrayHelper(seed);
